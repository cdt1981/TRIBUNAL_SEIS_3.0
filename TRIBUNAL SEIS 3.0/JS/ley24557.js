...
function sumaRemuneraciones_24557() {
    ...
    for (i = 0; i < misRemun_24557.length; i++) {
        ...
        // let resultNumber = Number(valor.replace(/\,/g, "."));
    ...
    document.getElementById("vibm").value = transformar(VIBM.ingresoBase);

    miResultadoFinalArray_24557.length = 0;

    // $("#btnCalcular_24557").attr("disabled", "true");
...
$(document).ready(function () {
    ...
    $("#quincenal").on("change", function (){
        if ($(this).is(":checked")){
                $("#sinRipte").attr("data-target", "#modal24557_quincenal");
                convertToString_24557();
                pasaMeses24557_quincena();
        } else {
                $("#sinRipte").attr("data-target", "#modal24557");
                muestra24557();
        };
    });
